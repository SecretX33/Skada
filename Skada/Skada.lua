...
do
	...
	local function say(msg, chan, chantype)
...
function Skada:Report(channel, chantype, report_mode_name, report_set_name, max, window)
	...
	Skada:QueueSendChat(string.format(L["Skada report on %s for %s, %s to %s:"], report_mode.title or report_mode:GetName(), report_set.name, date("%X",report_set.starttime), date("%X",endtime)), channel, chantype)
	...
	for i, data in ipairs(report_table.dataset) do
		if data.id then
			if report_mode.metadata and report_mode.metadata.showspots then
				Skada:QueueSendChat(("%2u. %s   %s"):format(nr, data.label, data.valuetext), channel, chantype)
			else
				Skada:QueueSendChat(("%s   %s"):format(data.label, data.valuetext), channel, chantype)
			end
			nr = nr + 1
		end
		if nr > max then
